// RUN: %clang_analyze_cc1 -Wno-array-bounds -analyzer-checker=core,alpha.security.BadScaledPointerArithmetic -verify %s
...
void noncompliant_f3(struct mystruct *msptr) {
  ...
  // this was impossible in tidy
  memset2(ptr + skip, 0, sizeof(struct mystruct) - skip);
 // expected-warning@-1 {{calculated from a sizeof}}
}
...
void noncompliant_f4(void) {
  ...
  while ( ptr < bptr + bufsize ) { // compliant
    *ptr = 0;
    ptr += sizeof(*ptr); // expected-warning {{calculated from a sizeof}}
  }
}
...
void noncompliant_f5(void) {
  ...
  while ( ptr < bptr + bufsize ) { // compliant
    *ptr = 0;
    ptr = ptr + sizeof(*ptr); // expected-warning {{calculated from a sizeof}}
  }
}
